...
import { WorkerInitCommand } from '../../src/commands/worker-init.js';

describe('WorkerInitCommand', () => {
  let command: WorkerInitCommand;

  beforeEach(() => {
    command = new WorkerInitCommand();
  });

  describe('readManifestFromEnv', () => {
  ...
  describe('getSessionId', () => {
    it('should read session ID from env', () => {
      ...
      try {
        process.env.MAESTRO_SESSION_ID = 'test-session-123';
  ...
  describe('validation', () => {
    it('should validate worker role', () => {
      ...
      const isValid = command.validateWorkerManifest(manifest);

      expect(isValid).toBe(true);
    });

    it('should reject orchestrator role', () => {
      const manifest = {
        manifestVersion: '1.0',
  ...
  describe('error handling', () => {
    ...
    it('should provide helpful error for wrong role', () => {
      const error = command.formatError('wrong_role', 'orchestrator');

      expect(error).toContain('worker');
      expect(error.toLowerCase()).toContain('orchestrator');
    });
  });
});
