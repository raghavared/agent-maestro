...
interface Task {
  ...
  // Model configuration
  model?: 'haiku' | 'sonnet' | 'opus';  // LLM model to use for this task
}
```
...
| `model` | string | No | LLM model: `haiku`, `sonnet`, or `opus` |
...
All entity IDs follow a consistent pattern generated by `TimestampIdGenerator.generate(prefix)`:
...
tmpl_1738713850000_f5g6h7i8j    # Template ID
